<template>
  <v-card :loading="loading">
    <v-card-title primary-title>
      <v-row no-gutters>
        ...
        <v-col v-show="selectedFiles.length > 0" class="shrink" cols="2">
          <v-badge
            color="primary"
            class="white--text"
    ...
    <v-card-text v-show="selectedFiles.length > 0">
      <v-row
        v-for="(file, index) in selectedFilesPreview"
      ...
      <v-row v-if="maxSelectedFilesReached" no-gutters>
        <v-col>...</v-col>
      </v-row>
    </v-card-text>
 ...
 -->
          <img
            :src="wgetDownloadInfo.image"
            :style="
              wgetDownloadInfo.style
                ? wgetDownloadInfo.style
          ...
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <v-btn
                ...
                :download="
                  wgetDownloadInfo.downloadFileName
                    ? wgetDownloadInfo.downloadFileName
                    : null
                "
                target="_blank"
      ...
      <v-row v-show="showDescription" align="center" class="pt-3" no-gutters>
        <v-col>
...
export default {
  ...
  data: () => ({
    ...
    fileSelectionDescription:
      'Select multiple files via the checkboxes, if a selected directory contains no files, nothing will be listed here. Subdirectories have to be opened first before a selection is possible. For directories with many files use the other download options.',
