        ...
        <v-row align="center" class="py-1" no-gutters>
          <v-col class="shrink">
            ...
            <img
              :src="tool.image"
              :style="
                tool.style ? tool.style : 'width: 40px; border-radius: 50%; '
              "
            />
            <!-- </v-btn>
              </template>
            ...
            <v-tooltip bottom>
              <template v-slot:activator="{ on, attrs }">
                <v-btn
                  ...
                  :href="tool.href ? tool.href : ''"
                  :download="
                    tool.downloadFileName ? tool.downloadFileName : null
                  "
                  target="_blank"
                  @click="
                    !tool.href && tool.clickCallback
                      ? tool.clickCallback()
                      : $emit('click')
                  "
                >
...
export default {
  ...
  data: () => ({
    ...
    entrieFolderDownloadDescription:
      'To download all files from a directory select it in the list, and choose the tools you would want to access this directory. There can be many files and large ones. If your interested in specific files use the file selection to download them.',
  }),
  methods: {
    clickShowDesc(tool) {
