<template>
  <v-card>
    <v-sheet class="pa-4" color="secondary">
      ...
      <v-row no-gutters align="center" justify="end" class="pt-2">
        <!-- <v-col >
    ...
    <v-card-text
      v-if="contentMapValues"
      :style="`max-height: ${height}; overflow:auto;`"
    >
      <TreeView
        :items="contentMapValues"
        :baseUrl="baseUrl"
        :search="search"
    ...
    <v-sheet v-if="!contentMapValues" class="pa-4">
      <v-card-text
        :style="`background-color: ${$vuetify.theme.themes.light.error};`"
      >
        {{ `No files or folder found for the prefix: ${prefix}` }}
      </v-card-text>

      <v-card-actions>
        <v-btn color="secondary" href="/"> Back to main site </v-btn>
      </v-card-actions>
    </v-sheet>
  </v-card>
</template>
